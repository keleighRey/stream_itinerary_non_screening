...
date: "`r Sys.Date()`"
params:
  data: BWRM_Deforest_Moodna.csv
  survey:  BWRM_Deforest_Moodna
  date:
    input: date
...
data<-read.csv(here::here(paste("outputs/2022/",value,sep = "")))
...
```{r table-function,echo=FALSE,warning=FALSE}
...
if(params$qa){
 label.site<-c("__________________-W-EQ","__________________-W","__________________-SEQ")
 time<-c("   :   ","   :   ","   :   ")
...
if(params$suite=="routine"){
  label2<-lables[rep(seq_len(nrow(lables)), 11), ]}

if(params$suite=="screening"){
  label2<-lables[rep(seq_len(nrow(lables)), 7), ]}
